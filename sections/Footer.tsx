...
export default function Footer() {
  ...
  return (
    <footer className="relative mt-4 overflow-hidden">
      {/* Glow line */}
      <div className="relative h-px mx-4 sm:mx-8">
      ...
      {/* Sparkles row (hidden on mobile to keep it clean) */}
      <div className="relative h-7 overflow-visible hidden sm:block" aria-hidden="true">
        {SPARKLE_POS.map((s, i) => (
          <motion.div
            key={i}
            className="absolute -top-2"
            style={{ left: s.x }}
            animate={{ y: [-4, -10, -4], opacity: [0.4, 1, 0.4], rotate: [0, 180, 360] }}
            transition={{ duration: 3, delay: s.delay, repeat: Infinity, ease: "easeInOut" }}
          >
            <SparkleIcon size={s.size} color={i % 2 === 0 ? "#FFB3D1" : "#C09AFF"} />
          </motion.div>
        ))}
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 sm:py-12">
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8 mb-8 sm:mb-10">
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center gap-1.5 text-xs sm:text-sm text-gray-400 font-semibold">
          ...
          {/* Quick links */}
          <div>
            <h4 className="font-black text-gray-700 mb-3 sm:mb-4 text-sm sm:text-base">Quick Links</h4>
            <div className="space-y-2">
              {FOOTER_LINKS.map((link) => (
                <motion.a
                  ...
                  className="block text-xs sm:text-sm text-gray-400 font-medium hover:text-pink-500 transition-colors cursor-pointer"
                  whileHover={{ x: 4 }}
                >
                  → {link.label}
                </motion.a>
            ...
            <div className="flex gap-2">
              {SOCIAL.map(({ icon: Icon, href, label }) => (
                <motion.a
                  ...
                  aria-label={label}
                  className="w-8 h-8 sm:w-9 sm:h-9 glass-card rounded-full flex items-center justify-center text-gray-400 hover:text-pink-500 border border-white/60 transition-all"
        ...
        {/* Bottom bar */}
        <div className="pt-5 sm:pt-6 border-t border-pink-100/60 flex flex-col sm:flex-row items-center justify-between gap-3 sm:gap-4">
          <div className="flex items-center gap-1 text-xs sm:text-sm text-gray-400 font-medium flex-wrap justify-center sm:justify-start">
            <span>Made with</span>
            <motion.span
          ...
          <div className="flex items-center gap-3 sm:gap-4">
            ...
            <motion.button
              onClick={scrollToTop}
              className="w-8 h-8 sm:w-9 sm:h-9 rounded-full btn-kawaii flex items-center justify-center"
      ...
      {/* Bottom glow strip */}
      <div
        className="absolute bottom-0 left-1/2 -translate-x-1/2 w-48 sm:w-64 h-0.5 bg-gradient-to-r from-transparent via-pink-300 to-transparent opacity-60 blur-sm"
        aria-hidden="true"
      />
    </footer>
  );
