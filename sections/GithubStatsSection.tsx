...
const STATS = [
  { label: "Total Commits", value: "1,247", icon: Activity, color: "from-pink-300 to-rose-300", emoji: "💻" },
  { label: "Repositories", value: "42", icon: GitFork, color: "from-purple-300 to-violet-300", emoji: "📦" },
  { label: "GitHub Stars", value: "1.2k", icon: Star, color: "from-yellow-300 to-amber-300", emoji: "⭐" },
  { label: "Followers", value: "320", icon: Users, color: "from-sky-300 to-blue-300", emoji: "👥" },
];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          {STATS.map((stat, i) => (
            <motion.div
              ...
              className="gradient-border rounded-2xl p-5 shadow-kawaii cursor-default"
            >
              <div className={`w-12 h-12 rounded-2xl bg-gradient-to-br ${stat.color} flex items-center justify-center text-xl mb-3 shadow-md`}>
                {stat.emoji}
              </div>
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          <motion.div
            ...
            className="lg:col-span-2 gradient-border rounded-3xl p-6 shadow-kawaii"
          >
            <div className="flex items-center gap-2 mb-4">
              <Activity className="w-5 h-5 text-pink-400" />
              <span className="font-black text-gray-800">Contribution Graph</span>
            ...
            <div className="overflow-x-auto">
              <div className="flex gap-1 min-w-max">
                {CONTRIBUTION_DATA.map((week, weekIdx) => (
                  <div key={weekIdx} className="flex flex-col gap-1">
                    {week.map((level, dayIdx) => (
                      <motion.div
                        key={dayIdx}
                        className={`w-3 h-3 rounded-sm ${CELL_COLORS[level]} cursor-pointer`}
                        initial={{ scale: 0, opacity: 0 }}
                        animate={inView ? { scale: 1, opacity: 1 } : {}}
                        transition={{
                          delay: (weekIdx * 7 + dayIdx) * 0.003 + 0.5,
                          duration: 0.2,
                        }}
          ...
          >
            <div className="flex items-center gap-2 mb-5">
              ...
              <span className="font-black text-gray-800">Top Languages</span>
            </div>
              ...
              {LANG_STATS.map((lang, i) => (
                <div key={lang.name} className="space-y-1.5">
                  <div className="flex justify-between">
                    ...
                    <span className="text-sm font-black text-pink-500">{lang.percent}%</span>
                  </div>
        ...
        <motion.div
          ...
          className="mt-6 gradient-border rounded-3xl p-6 shadow-kawaii"
        >
          <div className="flex items-center gap-2 mb-6">
            <TrendingUp className="w-5 h-5 text-pink-400" />
            <span className="font-black text-gray-800">Monthly Activity</span>
          ...
          <div className="flex items-end gap-2 h-28">
            {ACTIVITY_BARS.map((height, i) => (
              <div key={i} className="flex-1 flex flex-col items-center gap-1">
                <motion.div
                  className="w-full rounded-t-xl bg-gradient-to-t from-pink-300 to-purple-300 relative overflow-hidden"
                  style={{ height: `${height}%` }}
                  initial={{ scaleY: 0 }}
